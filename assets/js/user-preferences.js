...
function storageKey(userId) {
  return `timzee_user_preferences_v3:${userId || "guest"}`;
}
...
export function mergePreferences(value) {
  return deepMerge(DEFAULT_USER_PREFERENCES, value || {});
}

export function readLocalPreferences(userId = "guest") {
  try {
...
export function writeLocalPreferences(userId, preferences) {
  ...
  try {
    localStorage.setItem(storageKey(userId || "guest"), JSON.stringify(merged));
  } catch (_) {}
  if (typeof window !== "undefined") {
    window.dispatchEvent(new CustomEvent("timzee:preferences", { detail: merged }));
  }
  return merged;
}
...
export async function saveUserPreferences(userId, partial) {
  ...
  const existing = readLocalPreferences(userId);
  const merged = mergePreferences(deepMerge(existing, partial));
...
export function getPreference(path, fallback = undefined, userId = "guest") {
  ...
  let current = readLocalPreferences(userId);
