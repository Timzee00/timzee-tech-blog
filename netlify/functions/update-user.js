...
async function requireAdmin(supabase, token) {
  ...
  if (role !== "admin" && role !== "super") return { error: "Only admins can access this." };
...
exports.handler = async (event) => {
  ...
  const targetRole = resolveRole(authUserData.user);
  ...
  const trustedRole = resolveRole(authUserData.user);
