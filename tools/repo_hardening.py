...
# Listing: remove the disconnected Contact Seller stub, validate inquiry input,
# reduce over-fetching, and use the item's actual currency.
patch(
    "listing.html",
    [
        (
...
# Public/profile reads: explicitly select only fields needed by public UI.
...
patch(
    "assets/js/chat.js",
    [(r'\.from\("profiles"\)\.select\("\*"\)', f'.from("profiles").select("{profile_public}")')],
)
patch(
    "assets/js/post.js",
    [(r'\.from\("profiles"\)\s*\.select\("\*"\)', f'.from("profiles").select("{profile_public}")')],
)
patch(
...
# Marketplace library: stop returning every column for public listing cards/detail helpers.
...
# General multi-file safety: every existing JS multi-file collection must be capped,
# reset the input so the same file can be selected again, and dedupe by identity.
# The existing marketplace/admin implementations already handle repeated selections;
# this pass only hardens their file-selection event boundaries rather than changing
# avatar/story/single-attachment semantics.
for path in ["assets/js/admin.js", "marketplace.html"]:
    p = ROOT / path
    text = p.read_text(encoding="utf-8")
    if 'MAX_MEDIA_FILES' not in text and 'postMediaFiles' in text:
        text = text.replace('let postMediaFiles = [];', 'const MAX_MEDIA_FILES = 8;\nlet postMediaFiles = [];', 1)
        text = re.sub(r'postMediaFiles\s*=\s*\[\.\.\.postMediaFiles,\s*\.\.\.selected\]\s*;', 'postMediaFiles = [...postMediaFiles, ...selected].slice(0, MAX_MEDIA_FILES);', text)
        p.write_text(text, encoding="utf-8")

# Lightweight repo-wide static checks: no raw profiles select(*) and no obvious marketplace contact stub.
for path in ROOT.rglob('*.js'):
    if '.git' in path.parts or 'node_modules' in path.parts:
        continue
    text = path.read_text(encoding='utf-8', errors='ignore')
    if re.search(r'from\(["\']profiles["\']\)\s*\.select\(["\']\*["\']\)', text):
...
if 'alert("Opening contact with " + state.listing.seller_name)' in listing:
    raise SystemExit('Disconnected Contact Seller stub remains')

print('Hardening transformations and static checks passed.')
